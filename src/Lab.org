...
**** 函数说明
#+begin_src julia
  iscomplete(tree::BinarySearchTree)
#+end_src
- 输入一个二叉树
- 返回是否为完全二叉树
**** 测试案例
#+begin_src julia
  @testset "practise 5" begin
    newtree = createBSTree(Int)
    for i in [2, 1, 4, 3]
      insert!(newtree, i)
    end
  
    @test iscomplete(newtree) == false
  
    insert!(newtree, 5)
    @test iscomplete(newtree) == true
  end
#+end_src
**** 完整代码
#+begin_src julia
  function iscomplete(tree::BinarySearchTree)
    for treenode in bfsiterate(tree)
      if isnil(left(treenode)) && !isnil(right(treenode))
        return false
